...
function enrichRow(song, rank, old) {
  ...
  return {
    ...
    ptsPlus:
      ptsDiff === 0 ? "—" : ptsDiff > 0 ? `+${ptsDiff}` : String(ptsDiff),
    totalPts,
...
function buildSpotify(country = "global") {
  ...
  if (old?.rawHash === rawHash) return old;
...
export default async function Home({ searchParams }) {
  ...
  return (
    <main>
      ...
      <style>{`
        body { margin:0; background:white; font-family:Arial, Helvetica, sans-serif; font-size:13px; color:#000; }
        .page { padding:10px; width:100%; }
        .logoBox { background:#070719; width:660px; padding:28px 30px; margin-bottom:12px; }
        .logo { font-size:52px; font-weight:900; letter-spacing:-3px; }
        .logoWhite { color:white; }
        .logoGradient { background:linear-gradient(90deg,#2f8cff,#00d084); -webkit-background-clip:text; -webkit-text-fill-color:transparent; }
        .menu { display:grid; grid-template-columns:repeat(7,110px); width:770px; margin-bottom:20px; }
        .menu a { background:#000; color:white; border:1px solid white; text-align:center; font-weight:bold; padding:8px 0; text-decoration:none; }
        .tabs, .buttons { display:flex; flex-wrap:wrap; margin-bottom:18px; }
        .tabs a, .buttons a { color:white; border:1px solid white; font-weight:bold; padding:8px 24px; text-decoration:none; }
        .buttons a { background:#062c91; }
        .blue { background:#052c93; } .green { background:#008060; } .orange { background:#d88b00; } .red { background:#8b0000; } .purple { background:#5b188f; }
        h1 { font-size:21px; margin:0 0 18px 0; }
        .redlink { color:#990000; font-weight:bold; }
        .search { margin-bottom:15px; }
        .search input { padding:5px; width:280px; }
        .search button, .countrySelect { padding:5px 12px; }
        .countryBox { margin-bottom:15px; }
        .spotifyChart { width:760px; background:#f7f7f7; border:1px solid #d0d0d0; padding:14px; margin-bottom:20px; }
        .spotifyChart h2 { font-size:18px; margin:0 0 12px 0; }
        .barRow { margin-bottom:10px; }
        .barInfo { display:grid; grid-template-columns:45px 1fr 120px; gap:8px; font-size:12px; margin-bottom:3px; }
        .barTitle { overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
        .barStreams { text-align:right; font-weight:bold; }
        .barTrack { width:100%; height:12px; background:#e2e2e2; border-radius:20px; overflow:hidden; }
        .barFill { height:100%; background:linear-gradient(90deg,#1db954,#00d084); border-radius:20px; }
        .tableWrap { width:calc(100vw - 20px); overflow-x:auto; padding-bottom:15px; border-bottom:1px solid #ccc; }
        table { border-collapse:collapse; font-size:12px; min-width:1600px; }
        th { padding:5px 7px; text-align:left; border-bottom:2px solid #ccc; white-space:nowrap; background:white; }
        ...
        .song { min-width:390px; max-width:460px; overflow:hidden; text-overflow:ellipsis; }
        .rank { font-weight:bold; }
        .up { color:green; font-weight:bold; }
        .down { color:#b00000; font-weight:bold; }
      ...
      <div className="page">
        ...
        <h1>
          {source === "spotify"
            ? `Spotify Top 200 ${spotifyCountries.find((c) => c.code === country)?.label || "Global"}`
            : source === "index"
